      ...
      <tbody>
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
          <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
            ...
            <div class="text" v-if="child.eventName!=undefined">{{child.eventName | newPrice}}</div>
...
export default {
  ...
  methods: {
    ...
    getEvents(y, m, d) {
      ...
      let eventName = this.events[
        y + "-" + this.zeroPad(m) + "-" + this.zeroPad(d)
      ];
      let data = {};
      ...
      if (this.range) {
        if (this.rangeBegin.length == 0 || this.rangeEndTemp != 0) {
          ...
          begin.forEach((item, index) => {
            if (index === 1) {
              item = this.zeroPad(item + 1);
            } else {
              item = this.zeroPad(item);
            }
            b.push(item);
          });
          ...
          this.$emit("select", result, begin, end);
        }
        this.render(this.year, this.month);
      } else if (this.multi) {
        // 如果已经选过则过滤掉
        ...
        this.today = [k1, k2];
        this.eventName = this.days[k1][k2].eventName || "";

        let result = {
          dateTime:
            this.year +
            "-" +
            this.zeroPad(this.month + 1) +
            "-" +
    ...
    changeYear() {
      if (!this.changeYears) return false;
      if (this.yearsShow) {
